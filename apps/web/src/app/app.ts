    ...
    <div class="min-h-dvh bg-bg text-text font-sans">
      <header class="sticky top-0 z-10 bg-surface border-b border-border px-4 py-3 flex items-center justify-between max-w-xl mx-auto w-full min-h-[56px]">
        ...
        <div class="flex items-center gap-1">
          <a routerLink="/search" class="flex items-center justify-center w-11 h-11 text-muted hover:text-text rounded-lg hover:bg-border/30 transition-colors" [attr.aria-label]="'app.searchAriaLabel' | transloco">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
              <circle cx="11" cy="11" r="8"/><path stroke-linecap="round" d="M21 21l-4.35-4.35"/>
            </svg>
          </a>
          @if (auth.isLoggedIn()) {
            <button (click)="onExport()" class="flex items-center justify-center w-11 h-11 text-muted hover:text-text rounded-lg hover:bg-border/30 transition-colors" [attr.aria-label]="'app.exportAriaLabel' | transloco">
              <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1M8 12l4 4 4-4M12 4v12"/>
              </svg>
            </button>
          }
          <button (click)="toggleLang()" class="flex items-center justify-center min-w-11 h-11 text-xs font-mono text-muted hover:text-text rounded-lg hover:bg-border/30 transition-colors border border-border px-2">
            {{ 'app.langToggle' | transloco }}
          </button>
          @if (auth.isLoggedIn()) {
            <button (click)="onLogout()" class="flex items-center justify-center min-w-11 h-11 px-3 text-sm text-muted hover:text-text rounded-lg hover:bg-border/30 transition-colors">{{ 'app.logout' | transloco }}</button>
          } @else {
            <a routerLink="/login" class="flex items-center justify-center min-w-11 h-11 px-3 text-sm text-muted hover:text-text rounded-lg hover:bg-border/30 transition-colors">{{ 'app.login' | transloco }}</a>
            <a routerLink="/register" class="flex items-center justify-center h-11 px-4 text-sm bg-loop text-white rounded-lg hover:opacity-90 transition-opacity">{{ 'app.register' | transloco }}</a>
          }
        </div>
      </header>
